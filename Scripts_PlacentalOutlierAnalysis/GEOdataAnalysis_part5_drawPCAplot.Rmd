...
```{r}
## all samples
PCA551 <- ggplotPCA(PCA_Placenta_M = PCA_Placenta_M, Matched_pd = Matched_pd, value = "all551NormMAll")
...
All551rbind <- PCA551
All551rbind[is.na(All551rbind$FetalSex), ]$FetalSex <- rep("Female",2)

All551rbind$TrimesterSex <- do.call(paste0, All551rbind[c("Trimester", "FetalSex")])

PCAallplot <- All551rbind %>%
  ggplot(aes(x, y)) + 
  geom_point(aes(shape = TrimesterSex, col=TissueNoGreen), alpha = 1, size = 2)+ 
  scale_color_manual(name="Tissue", values = c("Amnion"="#000000", "Chorion"="#BD0026", 
                                               "Decidua"="#984EA3",  
                                               "Placenta"="#D95F02", "Impure placenta"="#1B9E77", 
  ...
  scale_x_continuous(breaks=c(-400,0, 400, 800), limits = c(-410, 1150))+
  guides(color=guide_legend(nrow = 3), shape=guide_legend(nrow = 2)) +
  # stat_ellipse(data =All551rbind, aes(x, y,col=Tissue),
...
OutlierDetail <- PercClassOutPut %>% 
  ...
  dplyr::left_join(PCA551[, c("SampleNames", "Outlier2")], by = "SampleNames") %>% 
...
Where the potencial mixed placenta sample locate:
```{r}

PercClassOutPutMclust$Sample_name[42:44]
...
datMethTraining <- datMeth_placenta[,ind] %>% cbind(NormBMlist$NormMAll[,!(NormBMlist$phenoDataAll$Sample%in%"Placenta")])

datMethTesting <- datMeth_placenta[,-ind] %>% cbind(NormBMlist$NormMAll[,!(NormBMlist$phenoDataAll$Sample%in%"Placenta")])
...
res2001 <- MclustTrainTestFUN(datMeth_placenta = datMeth_placenta)

# plot mclust result:
source(file = "~/DNAme/Process_decidual/TraningTestFuns.R")

ggarrange(
PlotClassFUN(df = res2001$df_training, PCA_traning = res2001$PCA_traning),
PlotClassFUN(df = res2001$df_test, PCA_traning = NULL), common.legend = TRUE)


TableTrainTestFUN(df=res2001$df_training)
TableTrainTestFUN(df=res2001$df_test)
