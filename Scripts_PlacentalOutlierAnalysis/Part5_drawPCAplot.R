...
PCA551plot <- PCA551 %>%
  ...
  # geom_text(aes(x=315 , y=-600, label="Q3+3*IQR"), vjust=-0.4, hjust=0, size=3) + #angle=90,, "Q3+3*IQR")
...
PCAT1plot <- PCA551_First %>%
  ...
  labs(x="PC1", y="PC2")+
  guides(color=guide_legend(nrow = 3), shape=guide_legend(nrow = 2)) +
  stat_ellipse(aes(x, y,col=Tissue), type = "norm") +
  theme(text = element_text(size=9),
        legend.title=element_text(size=9),
        legend.text=element_text(size=9),
        legend.position="top")
...
PCAT2plot <- PCA551_Second %>%
  ggplot(aes(x, y)) + 
  geom_point(aes(shape = Trimester, col=Tissue), alpha = 1, size = 2)+ 
  scale_color_manual(name="Tissue", values = c("#D95F02", "#1B9E77"))+
  scale_shape_manual(values=1)+
  labs(x="PC1", y="PC2")+
  guides(color=guide_legend(nrow = 3), shape=guide_legend(nrow = 2)) +
  stat_ellipse(aes(x, y,col=Tissue), type = "t") +
  theme(text = element_text(size=9),
        legend.title=element_text(size=6),
        legend.text=element_text(size=6),
        legend.position="top")

PCA551_Term$Tissue1 <- str_replace_all(PCA551_Term$Tissue, 
                                       c("Placental outliers"="Placenta", "Amnion|Chorion|Decidua"="Placental outliers"))

PCATermplot <- PCA551_Term %>%
  ggplot(aes(x, y)) + 
  geom_point(aes(shape = Trimester, col=Tissue), alpha = 1, size = 2)+ 
  scale_color_manual(name="Tissue", values = c("#000000", "#BD0026", "#984EA3","#D95F02", "#1B9E77", "#F781BF"))+
  scale_shape_manual(values=5)+
  labs(x="PC1", y="PC2")+
  guides(color=guide_legend(nrow = 3), shape=guide_legend(nrow = 2)) +
  stat_ellipse(data =PCA551_Term, aes(x, y,col=Tissue1), 
               type = "norm", show.legend =FALSE, inherit.aes = FALSE) +
  theme(text = element_text(size=9),
        legend.title=element_text(size=6),
        legend.text=element_text(size=6),
        legend.position="top")
