...
save(GSE66210.meta_v1, GSE74738.meta_v1, GSE69502.meta_v1, GSE75196.meta_v1, 
  ...
  if(EPICdata=="OurEPIC96"){
    
baseDir_96 <- file.path("/home/qianhui/DNAme/EPICarrayData/idat_files")
phenotypes <- read.metharray.sheet(baseDir_96)

phenotypes_GA6to9_10samples_inGEOformat <-  phenotypes %>% 
  dplyr::filter(Pool_ID%in%"Placental Tissue") %>% 
  inset('GEO_accession', value=rep("OurData", 96)) %>% 
  inset('Sample', value=rep('Placenta', 96)) %>% 
  mutate(Sample_name = str_replace_all(Basename, ".+/", "")) %>% 
  inset('Disease', value=rep("Uncomplicated", 96)) %>% 
  mutate(Gestation= as.character(Gestational.Age)) %>% 
  mutate(Trimester= str_replace_all(Trimester, "1st-Trimester", "First")) %>% 
  mutate(Additional_Info =Outliers) %>% 
  inset('Additional_Info_2', value=rep("NA", 96)) %>% 
  mutate(Fetal_Sex=str_replace_all(Fetal.Sex, c('M'='Male', 'F'='Female'))) %>% 
  inset('ArrayType', value=rep('EPIC', 96)) %>% 
  inset('Study', value=rep('ourStudy', 96)) %>% 
  dplyr::select(GEO_accession:Gestation, Trimester, Additional_Info:Study, Array, Slide)
...
save(GSE66210.meta_v1, GSE74738.meta_v1, GSE69502.meta_v1, GSE75196.meta_v1, 
   ...
   if(EPICdata=="OurEPIC131"){
    
# baseDir_131 <- file.path("/home/qianhui/DNAme/EPICarrayData/idat_files")
phenotypes <- readRDS(file = "/home/qianhui/DNAme/Process_placenta_workflow/RDSfiles/phenotypes_bind.rds")

phenotypes_GA6to9_10samples_inGEOformat <-  phenotypes %>% 
  inset('GEO_accession', value=rep("OurData", nrow(.))) %>% 
  inset('Sample', value=rep('Placenta', nrow(.))) %>% 
  mutate(Sample_name = str_replace_all(Basename, ".+/", "")) %>% 
  inset('Disease', value=rep("Uncomplicated", nrow(.))) %>% 
  mutate(Gestation= as.character(Gestation_num)) %>% 
  mutate(Trimester= str_replace_all(Trimester, "1st-Trimester", "First")) %>% 
  mutate(Additional_Info =Outliers) %>% 
  inset('Additional_Info_2', value=rep("NA", nrow(.))) %>% 
  mutate(Fetal_Sex=str_replace_all(FetalSex, c('M'='Male', 'F'='Female'))) %>% 
  inset('ArrayType', value=rep('EPIC',nrow(.))) %>% 
  inset('Study', value=rep('ourStudy', nrow(.))) %>% 
  dplyr::select(GEO_accession:Gestation, Trimester, Additional_Info:Study, Array, Slide)
...
OurOrYourDataPhenoFUN(EPICdata="OurEPIC131")
...
GEO_RGset_EPIC_normalPlacenta_ourStudy <- read.metharray.exp(base = GEO_baseDir, 
                                                             targets = GEO_phenotypes_EPIC_ourStudy, force = TRUE)
...
### other 85tissue types:
The other 85tissue types except for placenta, including:
