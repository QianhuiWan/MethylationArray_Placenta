...
pd <- Matched_pd[, c("Sample_name", "Sample","Trimester", "Fetal_Sex", "ArrayType", 
                     "Study", "Array", "Slide")]

# pd_term <- pd[pd$Trimester%in%"Term",]
# Mallterm <- Mall[, colnames(Mall) %in% pd_term$Sample_name]
...
pd_term[,cols2] <-  lapply(pd_term[,cols2], function(x){as.character(x)})

# SVD plots_all 408 samples
champ.SVD(beta = Mallterm, pd = pd_term,resultsDir = "/home/qianhui/DNAme/Process_decidual/figures/MPlot_allM408_SVD")
...
TrimesterAdjusted_Mall <- champ.runCombat(beta = Mall, 
                pd = pd,
                variablename = "Sample",
                batchname = c("ArrayType","Trimester"),
                logitTrans = FALSE)
