...
# file = "/home/qianhui/DNAme/figures-draftGA-analysis/PCA_Placenta_M_T1_Bvalues.rds")
# PCA_Placenta_M_T1 <- readRDS(file = "/home/qianhui/DNAme/figures-draftGA-analysis/PCA_Placenta_M_T1_Bvalues.rds")

PCA_Placenta_M_T1 <- PCA_PlacentaT1_M

DimPval <- dimdesc(PCA_Placenta_M_T1, axes = 1:2)
...
Calculate beta difference between placenta and other tissue types:

```{r}

PlacentaT1_names <- PhenodataPlacenta_T1[!(PhenodataPlacenta_T1$Additional_Info%in%"Outlier"),]$Sample_name

OutlierT1_names <- PhenodataPlacenta_T1[PhenodataPlacenta_T1$Additional_Info%in%"Outlier",]$Sample_name


Placenta_T1_MeanB <- Placenta_betaNorm_T1[rownames(Placenta_betaNorm_T1)%in%ProbeContrib.df.orderedTop2perc_T1$Name, 
                                      colnames(Placenta_betaNorm_T1)%in%PlacentaT1_names] %>% 
  rowMeans()

Outlier_T1_MeanB <- Placenta_betaNorm_T1[rownames(Placenta_betaNorm_T1)%in%ProbeContrib.df.orderedTop2perc_T1$Name, 
                                      colnames(Placenta_betaNorm_T1)%in%OutlierT1_names] %>% 
  rowMeans()
  

table(Outlier_T1_MeanB-Placenta_T1_MeanB >= 0.2)
...
PhenodataPlacenta_T1$Outlier <- factor(ifelse(PhenodataPlacenta_T1$Additional_Info%in%"Outlier", 
                                              "Outlier", "Standard"))
# PCA_Placenta_M, PCA_PlacentaT1_M, PCA_PlacentaTerm_M, we need PCA_Placenta_M
load(file = "/home/qianhui/DNAme/Process_decidual/RDSfiles/PCA_placenta_deciduals_allT1Term_BMval.RData")
...
CutOffRightNames <- PCA_Placenta_M$ind$coord[, 1][PCA_Placenta_M$ind$coord[, 1]>CutOffRight] %>% names
CutOffLeftNames <- PCA_Placenta_M$ind$coord[, 1][PCA_Placenta_M$ind$coord[, 1]<CutOffLeft] %>% names

PhenodataPlacenta_T1$OutlierMore <- factor(
  ifelse(PhenodataPlacenta_T1$Additional_Info%in%"Outlier", "ImpureOutlier",
         ifelse(PhenodataPlacenta_T1$Sample_name%in%CutOffRightNames, "PureOutlierRight", 
                ifelse(PhenodataPlacenta_T1$Sample_name%in%CutOffLeftNames, "PureOutlierLeft", "PurePlacenta"))))

phenoDataPlacenta.df_T1 <- PhenodataPlacenta_T1 %>% as.data.frame(stringsAsFactors=FALSE) %>%
  dplyr::select(c("Sample_name", "Outlier", "OutlierMore"))
...
# plot each outleir
outlierSampNamesT1 <- placenta_beta_dim1Top2perc.df_T1[placenta_beta_dim1Top2perc.df_T1$Outlier%in%"Outlier",]$Sample_name %>% unique()

T1outliers <- placenta_beta_dim1Top2perc.df_T1[placenta_beta_dim1Top2perc.df_T1$Sample_name%in%outlierSampNamesT1,]
T1outliers %>% ggplot(aes(x=Sample_name, y=value))+geom_boxplot()

# plot boxplot
outlierDim1Plot_T1 <- placenta_beta_dim1Top2perc.df_T1 %>% 
  ggplot(aes(x=Outlier, y=value))+
  geom_boxplot()+
  scale_x_discrete(labels=c("Outlier"="Impure", "Standard"="Other Samples"))+
...
# subset placenta data (T2) and use PCA1
```{r T2}
...
# check subseted columns are placental samples or not
table(PhenodataPlacenta_T2$Sample_name==colnames(Placenta_Mnorm_v1_T2))
## PCA for normalised M values
# PCA_Placenta_M_T2 <- FactoMineR::PCA(base::t(Placenta_Mnorm_v1_T2))
### plot eigen values for each PCA
# barplot(PCA_Placenta_M_T2$eig[,1], main="Eigenvalues", names.arg=1:nrow(PCA_Placenta_M_T2$eig))
# plot(PCA_Placenta_M,choix="ind")
# calculate correlation for each probe for PCA1 and PCA2
# PCA_Placenta_M_probesSigPCA1and2 <- dimdesc(PCA_Placenta_M, axes =1:2, proba = 0.01)#444312/614861

PCA_Placenta_M_T2 <- readRDS(file = "/home/qianhui/DNAme/figures-draftGA-analysis/PCA_Placenta_M_T2_Bvalues.rds")

```


## the contribution of probes to PC1 (T2 placenta samples)
```{r ContributionT2Samples}
# contribution of probs in Dim1
ProbeContrib.df_T2 <- PCA_Placenta_M_T2$var$contrib %>% as.data.frame() %>% 
  rownames_to_column(var="Name")
###### quick look at conribution percentages:
summary(ProbeContrib.df_T2)
###### get Dim1 top 2% probes by ordering the contribution percentages of Dim1
ProbeContrib.df.orderedTop2perc_T2 <- ProbeContrib.df_T2[base::order(-ProbeContrib.df_T2$Dim.1),] %>% 
   dplyr::slice(1:(0.02*nrow(ProbeContrib.df_T2)))
...
# GO analysis
Dim1_top2perc_T2 <- ProbeContrib.df.orderedTop2perc_T2$Name
goMeth_Dim1_top2perc_T2 <- gometh(sig.cpg = Dim1_top2perc_T2,
                           all.cpg = rownames(Placenta_Mnorm_v1_T2),
                           collection = "GO", array.type = "450K", prior.prob = TRUE, ann=ann450k)
goMeth_Dim1_top2perc_T2[goMeth_Dim1_top2perc_T2$FDR<0.05,] %>% dim()
topGO(goMeth_Dim1_top2perc_T2, number = 10)

# plot Top GO terms
## con30
top2perc_GO_T2 <- topGO(goMeth_Dim1_top2perc_T2, number = 20) %>% rownames_to_column(var="GO_ID")
top2perc_GO_T2$Terms <- factor(top2perc_GO_T2$Term, levels = rev(top2perc_GO_T2$Term))
theme_set(theme_bw())
top2perc_GOPlot_T2 <- ggplot(top2perc_GO_T2, aes(x= Terms, y= -log10(FDR))) + #fill=Ont
  geom_bar(stat='identity')+ #"midnightblue"
  # scale_fill_hue()+
  coord_flip()+
  geom_hline(yintercept = -log10(0.05), colour="red")+
...
tiff(file = "/home/qianhui/DNAme/Process_decidual/figures/Placenta_top2perc_GOPlot_ForOutlier_T2_residual.tiff",
    width = 20, height = 12, units = "cm", res = 300)
top2perc_GOPlot_T2
dev.off()
...
## plot box plot for beta values of Dim1_top2perc porbes
## manipulate data frames
PhenodataPlacenta_T2$Outlier <- factor(ifelse(PhenodataPlacenta_T2$Additional_Info%in%"Outlier", 
                                              "Outlier", "Standard"))
...
placenta_beta_dim1Top2perc_T2 <- Placenta_betaNorm_T2[rownames(Placenta_betaNorm_T2)%in%Dim1_top2perc_T2,]
...
outlierDim1Plot_T2 <- placenta_beta_dim1Top2perc.df_T2 %>% 
  ggplot(aes(x=OutlierMore, y=value))+
  geom_boxplot()+
...
* The results showed that this one outlier from second trimester was not an impure placenta sample, it's just because it was dissected the maternal side of placenta.
...
PCA_Placenta_M_Term <- readRDS(file = "/home/qianhui/DNAme/figures-draftGA-analysis/PCA_Placenta_M_Term_residuals.rds")
...
# contribution of probs in Dim1
ProbeContrib.df <- PCA_Placenta_M_Term$var$contrib %>% as.data.frame() %>% 
  rownames_to_column(var="Name")
###### quick look at conribution percentages:
...
## plot box plot for beta values of Dim1_top2perc porbes
## manipulate data frames
PhenodataPlacenta$Outlier <- factor(ifelse(PhenodataPlacenta$Additional_Info%in%"Outlier",
                                        "Outlier", "Standard"))

PhenodataPlacenta$OutlierMore <- PhenodataPlacenta$OutlierMore <- factor(
  ifelse(PhenodataPlacenta$Additional_Info%in%"Outlier", "ImpureOutlier",
         ifelse(PhenodataPlacenta$Sample_name%in%CutOffRightNames, "PureOutlierRight", 
                ifelse(PhenodataPlacenta$Sample_name%in%CutOffLeftNames, "PureOutlierLeft", "PurePlacenta"))))
...
# plot each outleir (contain GSM1947213_6008581028_R01C01, supposed to be sampled form maternal side of placenta)
outlierSampNames <- placenta_beta_dim1Top2perc.df[placenta_beta_dim1Top2perc.df$OutlierMore%in%"PureOutlierRight",]$Sample_name %>% unique()

t <- placenta_beta_dim1Top2perc.df[placenta_beta_dim1Top2perc.df$Sample_name%in%outlierSampNames,]
t %>% ggplot(aes(x=Sample_name, y=value))+geom_boxplot()
...
outlierDim1Plot_Term <- placenta_beta_dim1Top2perc.df %>% 
  ggplot(aes(x=OutlierMore, y=value))+
  geom_boxplot()+
  scale_x_discrete(labels=c("Outlier"="Impure", "Standard"="Pure"))+
  labs(x="", y="Beta values")+
  theme(axis.text.y = element_text(size=12, colour ="black"),
        axis.text.x = element_text(size=12, colour ="black"),
        axis.title=element_text(size=12, colour ="black"),#,face="bold"
...
saveRDS(placenta_beta_dim1Top2perc.df, file = "/home/qianhui/DNAme/Process_decidual/RDSfiles/placenta_beta_dim1Top2perc_df_Term.rds")
...
# plot to show that outliers that not clustered with other tissue are placenta samples
```{r}
...
# PCAs <- readRDS(file = "/home/qianhui/DNAme/Process_decidual/figures/T1TermSamples_PCA408_3plots.rds")

# CorBoxplot <- readRDS(file = "/home/qianhui/DNAme/Process_decidual/RDSfiles/Placenta_Mnorm_T1Term_CorBoxplot.rds")
# we don't need desity plot in Figure1
...
# we don't need OiutlierPerPlot in Figure1
# OutlierPerPlot <- readRDS(file = "/home/qianhui/DNAme/Process_decidual/figures/OutlierPerPlot.rds")
...
# densityCorBoxplots <- ggarrange(density551, OutlierPerPlot, 
#           labels = c("G", "H"), 
...
PC1plots <- ggarrange(outlierDim1Plot_T1Term, top2perc_GOPlot_T1Term, #densityCorBoxplots, 
          labels = c("E", "F", NULL),
          font.label = list(size = 12, face = "bold", color ="black"),
          ncol = 3, widths = c(0.8,2,1.4))


tiff(file = "/home/qianhui/DNAme/Process_decidual/figures/TechFigure1_v2.tiff",
