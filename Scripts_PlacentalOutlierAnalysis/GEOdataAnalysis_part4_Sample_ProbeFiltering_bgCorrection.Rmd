...
FilterFun <- function(RGsetChara, crossreactiveProbes, annotation, tissueType, 
  ...
  if(ncol(RGset) ==1){
    ...
    # methylset_raw <- preprocessNoob(RGset, offset = 15, dyeCorr = TRUE, verbose = FALSE,
    #            dyeMethod="single")
...
## calculate M values
    Join.df_M <- log2(Join.df_v2/(1 - Join.df_v2))
    Join.df_M_v1 <- Join.df_M[apply(Join.df_M, 1, function(x) all(is.finite(x))), ]
...
# get PCA result
PCA_all408_bgCorrected <- FactoMineR::PCA(base::t(Join.df_M_v1))

save(Join.df_v2, Join.df_M_v1, GEO_phenotypes_joinedSamples, MDS_joinedSamples, 
...
GEO_Mset_450k_Tissue <- preprocessRaw(GEO_RGset_450k_Tissue)

GEO_Mset_EPIC_Tissue <- preprocessRaw(GEO_RGset_EPIC_Tissue)
...
GEO_Mset_450k_Tissue_Fil <- GEO_Mset_450k_Tissue[rownames(GEO_Mset_450k_Tissue)%in%rownames(Join.df_v2),]
RemovedProbe_450k <- GEO_Mset_450k_Tissue[!(rownames(GEO_Mset_450k_Tissue)%in%rownames(Join.df_v2)),] %>% rownames()


GEO_Mset_EPIC_Tissue_Fil <- GEO_Mset_EPIC_Tissue[rownames(GEO_Mset_EPIC_Tissue)%in%rownames(Join.df_v2),]
RemovedProbe_EPIC <- GEO_Mset_EPIC_Tissue[!(rownames(GEO_Mset_EPIC_Tissue)%in%rownames(Join.df_v2)),] %>% rownames()
...
GEO_Mset_450k_Tissue_Fil_enmix <- preprocessENmix(GEO_RGset_450k_Tissue, exCpG = RemovedProbe_450k, nCores = 10)

GEO_Mset_EPIC_Tissue_Fil_enmix <- preprocessENmix(GEO_RGset_EPIC_Tissue, exCpG = RemovedProbe_EPIC, nCores = 10)
...
for (i in c("GEO_Mset_450k_Tissue_Fil_ENQN1","GEO_Mset_EPIC_Tissue_Fil_ENQN1")){
    Mset <- get(i)
...
Beta_all <- betaList$GEO_Mset_450k_Tissue_Fil_ENQN1 %>% 
  left_join(betaList$GEO_Mset_EPIC_Tissue_Fil_ENQN1, by="Probe") %>% 
  column_to_rownames(var="Probe") %>% 
  as.matrix()

M_all <- Mlist$GEO_Mset_450k_Tissue_Fil_ENQN1 %>% 
  left_join(Mlist$GEO_Mset_EPIC_Tissue_Fil_ENQN1, by="Probe") %>% 
  column_to_rownames(var="Probe") %>% 
  as.matrix()
...
```{r}
Matched_pd <- GEO_phenotypes_joinedSamples[match(colnames(M_all), GEO_phenotypes_joinedSamples$Sample_name),]
```
...
```{r}
NameWithoutGSE71678 <- Matched_pd[!(Matched_pd$Study%in%"GSE71678"),]$Sample_name
...
Mall_pcaRES %>% 
  ...
  geom_point(aes(shape = Tissue, col=Study), alpha = 1, size = 5)+ 
  # scale_color_manual(values = namedPal2)+
   geom_vline(aes(xintercept=quantile(PCA_all$ind$coord[rownames(PCA_all$ind$coord)%in%NameWithoutGSE71678, 1], 0.75)+ 1.5*IQR(PCA_all$ind$coord[rownames(PCA_all$ind$coord)%in%NameWithoutGSE71678, 1])),
             linetype=4, colour="black")+
  geom_vline(aes(xintercept=quantile(PCA_all$ind$coord[rownames(PCA_all$ind$coord)%in%NameWithoutGSE71678, 1], 0.25)+ 3*IQR(PCA_all$ind$coord[rownames(PCA_all$ind$coord)%in%NameWithoutGSE71678, 1])),
             linetype=4, colour="black")+
  scale_shape_manual(values=1:11)+
  labs(x="PC1", y="PC2")+
...
MAllNorm <- BetaAllNorm
table(colnames(MAllNorm) %in% Matched_pd$Sample_name)
# Matched_pd <- Matched_pd[Matched_pd$Sample%in%"Placenta",]
Mall <- MAllNorm[, colnames(MAllNorm)%in%Matched_pd$Sample_name]
