...
M_placenta <- NormBMlist$NormMAll[,NormBMlist$phenoDataAll$Sample%in%"Placenta"]

table(colnames(M_placenta) %in% GEO_phenotypes$Sample_name)

Matched_pd <- GEO_phenotypes[match(colnames(M_placenta), GEO_phenotypes$Sample_name),]
...
## all sampoes
PCA_placenta <- ggplotPCA(PCA_Placenta_M = PCA_Placenta_M, Matched_pd = Matched_pd, value = "allPlacenta")

PCA_addPostion <- PCA_placenta %>%
  dplyr::mutate(Position=str_replace_all(Study, c(
    "GSE69502|GSE100197|GSE115508|GSE98224|ourStudy|GSE74738|GSE66210"="Villous",
    ...
    "GSE75196"="Fetal side of placenta"))) #fetal side of placenta 

PCA_addPostion$Position[which(PCA_addPostion$Tissue=="Impure placenta")] <- rep("Impure placenta", 11)


PCA_Plot <- PCA_addPostion %>%
  ggplot(aes(x, y)) + 
  # geom_point(aes(shape = Tissue, col=Trimester), alpha = 1, size = 5)+ 
  geom_point(aes(shape = Trimester, col=Position), alpha = 1, size = 2)+ 
  scale_color_manual(name="Tissue", values = 
                       c("1cm below chorionic plate"="#000000", "2cm from umbilical cord"="#377EB8", 
                         "Adjacent to cord insertion"="#BD0026", 
                         "Fetal side of placenta"="#A6761D", "Impure placenta"="#1B9E77", 
                         "Villous"="#F781BF"))+
  scale_shape_manual(values=c(0,1,5))+
  labs(x="PC1 (16.61%)", y="PC2 (10.52%)")+
  ...
  #            linetype=4, colour="black")+
  # geom_text(aes(x= 480, y=-1000, label="Q3+1.5*IQR"), vjust=-0.4, hjust=0, size=3) + #angle=90,, "Q3+3*IQR")
  # geom_text(aes(x=-600, y=-1000, label="Q1-1.5*IQR"), vjust=-0.4, hjust=0, size=3) + #angle=90,, "Q3+3*IQR")
  stat_ellipse(aes(x, y,col=Position), type = "norm", level = 0.9) +
...
tiff(file = "/home/qianhui/DNAme/Process_decidual/figures/PCA_allPlacenta_position.tiff", 
     width = 20, height = 15, units="cm",  res = 300)
theme_set(theme_pubr())
PCA_Plot
dev.off()

saveRDS(PCA_Plot, file = "/home/qianhui/DNAme/Process_decidual/figures/PCA_Plot_allPlacenta.rds")
