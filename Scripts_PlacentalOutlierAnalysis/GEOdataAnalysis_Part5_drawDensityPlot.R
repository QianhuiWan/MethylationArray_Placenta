...
PhenodataPlacenta$Sample2 <- str_replace_all(PhenodataPlacenta$Sample, c(".+lacent.+"="Placenta", "umbilical cord"="Umbilical cord"))
PhenodataPlacenta$Sample2[which(PhenodataPlacenta$Outlier=="Outlier")] <- rep("Placental outliers",9)
...
Placenta_betaNorm_SampleGroupMeanDF <- NormBMlist$NormBAll %>% t() %>% dplyr::as_data_frame() %>% 
                                       inset('Sample2', value=PhenodataPlacenta$Sample2) %>% 
                                       reshape2::melt() %>% 
                                       group_by(Sample2, variable) %>% 
                                       summarise(SampleGroupMean=mean(value))
...
density551 <- Placenta_betaNorm_SampleGroupMeanDF %>% 
  ggplot(aes(x=SampleGroupMean, color=Sample2))+geom_density(aes(linetype=Sample2),size=1)+
