...
```
* There are 1971 probes from top 2 percent probes (6016) from PC1 that located in PMDs.
...
OutlierTerm <- tibble(
  ...
  Outlier_Term = paste0("Outlier_Term_", 1:6))
...
2% probes and Trimester:
```{r}

# placenta_beta_dim1Top2perc.df_all %>% 
...
kruskal.test(placenta_beta_dim1Top2perc.df_all$value~placenta_beta_dim1Top2perc.df_all$Trimester)

pairwise.wilcox.test(placenta_beta_dim1Top2perc.df_all$value,
                     placenta_beta_dim1Top2perc.df_all$Trimester, p.adjust.method = "BH")
...
library("ggpubr")
ggline(placenta_beta_dim1Top2perc.df_all, x = "Fetal_Sex", y = "value", 
       add = c("mean_se"), 
       order = c("Female", "Male"),
       ylab = "Beta", xlab = "Sex")+
  # ylim(0,1)
...
wilcox.test(placenta_beta_dim1Top2perc.df_all$value~placenta_beta_dim1Top2perc.df_all$Fetal_Sex)

```

2% probes and batch (Study):
```{r}
...
library("ggpubr")
ggline(placenta_beta_dim1Top2perc.df_all[!(placenta_beta_dim1Top2perc.df_all$Outlier.y%in%"Outlier"),],
       x = "Study", y = "value", 
       add = c("mean_se"), 
       # order = c("Female", "Male"),
       ylab = "Beta", xlab = "Study")+
  # ylim(0,1)
  scale_y_continuous(breaks=seq(0.0, 1, 0.2), limits=c(0, 1))+
  geom_hline(yintercept = c(0.35,0.5))

pairwise.wilcox.test(placenta_beta_dim1Top2perc.df_all$value,
                     placenta_beta_dim1Top2perc.df_all$Study, p.adjust.method = "BH")
