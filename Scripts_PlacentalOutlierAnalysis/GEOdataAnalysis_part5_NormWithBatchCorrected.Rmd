...
registerDoParallel(cores = 10)
  ...
  # saveRDS(NormBMlist, file = "/home/qianhui/DNAme/Process_decidual/RDSfiles/NormBMlist_131.rds")
...
for(i in TissueBMvalInfo$TissueTypes){
  ...
  plotMDSFUN(Normal_betaNorm = NormBMlist$NormBAll, 
             Tissue = i, 
             PhenoData = NormBMlist$phenoDataAll)
}
...
MDSnormCali408_Beta <- plotAll(BMvalue = normCali_BMof9TissueTypes_CaliTogether$Beta,
                           GEO_phenotypes_joinedSamples = normCali_BMof9TissueTypes_CaliTogether$BetaPhenoData)

MDSnormCali408_M <- plotAll(BMvalue = normCali_BMof9TissueTypes_CaliTogether$M, 
                        GEO_phenotypes_joinedSamples = normCali_BMof9TissueTypes_CaliTogether$BetaPhenoData)
...
# MDSnormCali408_M modified BMIQ calibrated data
...
## plot MDS plot
par(mfrow=c(1,1))
MDS_normCorrect408.Beta <- plotMDS(BMall_combat$Beta, top=nrow(BMall_combat$Beta), 
                              labels= BMall_combat$PhenoData$Sample, gene.selection="common") 

par(mfrow=c(1,1))
MDS_normCorrect408.M <- plotMDS(BMall_combat$M, top=nrow(BMall_combat$M), 
                              labels= BMall_combat$PhenoData$Sample, gene.selection="common") 


AllMMDSinfo <- ggplotMDS(MDS_norm408 = MDS_normCorrect408.M, 
          GEO_phenotypes_norm408 = BMall_combat$PhenoData, 
          value = "correctedMv1")

  
save(BMall_combat, MDS_normCorrect408.Beta, MDS_normCorrect408.M, 
     file = "/home/qianhui/DNAme/Process_decidual/RDSfiles/GEO_joinedSamples_NormBatchCorrect408_BetaM_MDS_OutlierConsidered.RData")
...
```{r useggplotMDSagain}
# for MDS_normCorrect408_Beta
AllBetaMDSinfo <- ggplotMDS(MDS_norm408 = MDS_normCorrect408.Beta, 
          GEO_phenotypes_norm408 = BMall_combat$PhenoData, 
          value = "correctedBeta")
...
## A) Use MDS_norm408.M and MDS_norm408.beta objects in `NormBMlist` list: batch not corrected
```{r}

# NormBMlist <- readRDS(file = "/home/qianhui/DNAme/Process_decidual/RDSfiles/NormBMlist.rds")
NormBMlist <- readRDS(file = "/home/qianhui/DNAme/Process_decidual/RDSfiles/NormBMlist_131.rds")

# MDS_norm408.M or NormBMlist$MDS_norm408.M
table(rownames(NormBMlist$MDS_norm408.M$distance.matrix)==NormBMlist$phenoDataAll$Sample_name)
MDSinfo_uncorrected <- ggplotMDS(MDS_norm408 = NormBMlist$MDS_norm408.M, 
                                 GEO_phenotypes_norm408 = NormBMlist$phenoDataAll, 
...
MDSinfo_uncorrected %>%
  ...
  geom_point(aes(shape = Study, col=Trimester), alpha = 1, size = 5)+ 
  # scale_color_manual(values = namedPal2)+
  scale_shape_manual(values=1:length(MDSinfo_uncorrected$Study))+
  labs(x="Dimension 1", y="Dimension 2")+
  theme(text = element_text(size=15))

# MDS_norm408.Beta or NormBMlist$MDS_norm408.Beta
table(rownames(NormBMlist$MDS_norm408.Beta$distance.matrix)==NormBMlist$phenoDataAll$Sample_name)
MDSinfo_uncorrected_beta <- ggplotMDS(MDS_norm408 = NormBMlist$MDS_norm408.Beta, 
                                      GEO_phenotypes_norm408 = NormBMlist$phenoDataAll, 
...
# save plots
tiff(file = "/home/qianhui/DNAme/Process_decidual/figures/allSamplesMDS408_notCorrected.tiff",
...
pdf(file = "/home/qianhui/DNAme/Process_decidual/figures/allSamplesMDS408_notCorrected.pdf", 
    width = 13, height = 7)
theme_set(theme_bw())
MDSinfo_uncorrected %>%
  ggplot(aes(x, y)) + 
...
## B) Use `quantro` to look at global differences between tissue types (all 408 samples)
...
# check sample (408 samples) order first
...
MDSinfo_NormBAll_1st <- ggplotMDS(MDS_norm408 = MDS_NormBAll_1st, 
                               GEO_phenotypes_norm408 = Sample_1st, 
...
MDSinfo_NormBAll_term <- ggplotMDS(MDS_norm408 = MDS_NormBAll_term, 
                                   GEO_phenotypes_norm408 = Sample_term, 
...
MDSinfo_NormMAll_1st <- ggplotMDS(MDS_norm408 = MDS_NormMAll_1st, 
                               GEO_phenotypes_norm408 = Sample_1st, 
...
MDSinfo_NormMAll_term <- ggplotMDS(MDS_norm408 = MDS_NormMAll_term, 
                                   GEO_phenotypes_norm408 = Sample_term, 
...
MDSinfoT1_combat <- ggplotMDS(MDS_norm408 = MDS_T1_combat, 
                    GEO_phenotypes_norm408 = Sample_1st, 
...
MDSinfoTerm_combat <- ggplotMDS(MDS_norm408 = MDS_Term_combat, 
                    GEO_phenotypes_norm408 = BM_Term_combat$PhenoData, 
...
MDSinfoT1 <- ggplotMDS(MDS_norm408 = MDS_T1, 
                    GEO_phenotypes_norm408 = Sample_1st, 
...
MDSinfoTerm <- ggplotMDS(MDS_norm408 = MDS_Term, 
                    GEO_phenotypes_norm408 = Sample_term, 
                    value = "Term_M_removeBatchEffect")
...
# 6. regress out batch estimated from control probes (whole data set, 408 smaples)

## plot batch corrected data (batch estimated based on control probes)
```{r}

# load the data of control probes
# Control408_Beta, Control408_M
load(file = "/home/qianhui/DNAme/Process_decidual/RDSfiles/Controls_BM_all408.RData")

## Response MDS$x varies with control PC1:
table(NormBMlist$phenoDataAll$Sample_name==rownames(Control408_M$PCA_control$ind$coord))
table(colnames(NormBMlist$NormMAll)==rownames(Control408_M$PCA_control$ind$coord))
cor.test(NormBMlist$MDS_norm408.M$x, Control408_M$PCA_control$ind$coord[,1])

## Fit model:
NormBMlist$phenoDataAll$ControlPC1 <- Control408_M$PCA_control$ind$coord[,1]
NormBMlist$phenoDataAll$ControlPC2 <- Control408_M$PCA_control$ind$coord[,2]
# NormBMlist$phenoDataAll$ControlPC3 <- Control408_M$PCA_control$ind$coord[,3]
...
MDSinfo_residuals <- ggplotMDS(MDS_norm408 = MDS_residuals, 
                               GEO_phenotypes_norm408 = NormBMlist$phenoDataAll, 
                               value = "MDS_residuals")
...
# save plots
tiff(file = "/home/qianhui/DNAme/Process_decidual/figures/allSamplesMDS408_controlProbeCorrected.tiff",
    width = 20, height = 12, units = "cm", res = 300)
theme_set(theme_bw())
MDSinfo_residuals %>%
  ggplot(aes(x, y)) + 
...
pdf(file = "/home/qianhui/DNAme/Process_decidual/figures/allSamplesMDS408_controlProbeCorrected.pdf", 
    width = 13, height = 7)
theme_set(theme_bw())
MDSinfo_residuals %>%
  ggplot(aes(x, y)) + 
...
<!-- INCs <- Control408_M$JoinAllControl -->
...
# 7. SVA to remove batch (whole data set, 408 smaples)
```{r}
...
# add detection P values for all beta values from our dataset
